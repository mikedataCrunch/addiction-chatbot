...
class ELI5DatasetQARetriver(Dataset):
    ...
    def make_example(self, idx):
        ...
        if self.training:
            ...
            # randomly choose an answer (multiple answer questions), per word split
            answer_tab = choice(answers).split(" ")
            
            # get a random slice from the answer, constrained by min_length
            start_idx = randint(0, max(0, len(answer_tab) - self.min_length))
            
            # new answer slice in string format
            answer_span = " ".join(answer_tab[start_idx:])
        else:
            # if using dataset as validation, take the first full answer text
...
class RetrievalQAEmbedder(torch.nn.Module):
    def __init__(self, sent_encoder, dim):
        ...
        self.sent_encoder = sent_encoder # currently, BERT encoder
        self.output_dim = 128
        
        # for learning of q&a projection matrices
...
def make_qa_retriever_model(model_name="google/bert_uncased_L-8_H-512_A-8", from_file=None, device="cuda:0"):
    ...
    # LongTensor is a 64-bit integer (signed)
...
def make_qa_s2s_batch(qa_list, tokenizer, max_len=64, max_a_len=360, device="cuda:0"):
    q_ls = [q for q, a in qa_list]
    a_ls = [a for q, a in qa_list]
    q_toks = tokenizer.batch_encode_plus(q_ls, max_length=max_len, padding=True, truncation=True)
    q_ids, q_mask = (
        torch.LongTensor(q_toks["input_ids"]).to(device),
        torch.LongTensor(q_toks["attention_mask"]).to(device),
    )
    a_toks = tokenizer.batch_encode_plus(a_ls, max_length=min(max_len, max_a_len), padding=True, truncation=True)
    a_ids, a_mask = (
        torch.LongTensor(a_toks["input_ids"]).to(device),
        torch.LongTensor(a_toks["attention_mask"]).to(device),
    )
...
def embed_questions_for_retrieval(q_ls, tokenizer, qa_embedder, device="cuda:0"):
    ...
    with torch.no_grad(): # what does this do?
...
def make_qa_dense_index(
    ...
    for i in range(n_batches):
        ...
        print(f"Batch: {i}, Elapsed: {time() - st_time} seconds")
#         if i % 50 == 0:
#             print(f"Batch: {i}, Elapsed: {time() - st_time} seconds")
